...
class MyValidator(wx.Validator):
    ...
    def Validate(self, win):
        ...
        if self.flag == ALPHA_ONLY:
        ...
        elif self.flag == SIGNED_FLOAT_ONLY:
            for x in val:
            ...
            if((val[0]!='-')and(val[0]!='.')):
                try:
                    ...
                    return False
        elif self.flag == EXPRESS_ONLY:
            B,B1,B2,B3,B4=60,60,60,60,60
            H,W,H3,W3=600,400,40,40
            kr,z,offs=10,10,10
            for x in val:
                if((x not in string.digits)and(x not in string.letters)):
                    return False
            if((val[0]!='-')and(val[0]!='.')):
                try:
                    ls = eval(val)
    ...
    def OnChar(self, event):
        ...
        if self.flag == SIGNED_FLOAT_ONLY and chr(key) in string.digits+'.'+'-'+'+':
            event.Skip()
            return
        if self.flag == ID_ONLY and chr(key) in string.digits+'x'+'X':
            event.Skip()
            return
